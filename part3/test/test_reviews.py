import pytest
from app import create_app
from app.extensions import db
from app.models.user import User
from app.models.place import Place

@pytest.fixture
def client():
    app = create_app('config.TestingConfig')
    with app.test_client() as client:
        with app.app_context():
            db.create_all()
            # إنشاء مستخدم ومكان لتقييمه
            user = User(email="critic@hbnb.io", first_name="Critic", last_name="User")
            user.hash_password("pass123")
            db.session.add(user)
            
            place = Place(title="Test Place", description="Test", price=100.0, latitude=1.0, longitude=1.0, owner_id=1)
            db.session.add(place)
            
            db.session.commit()
            yield client
            db.session.remove()
            db.drop_all()

def test_create_review(client):
    """اختبار كتابة تقييم على مكان"""
    # تسجيل الدخول
    login = client.post('/api/v1/auth/login', json={"email": "critic@hbnb.io", "password": "pass123"})
    token = login.get_json().get("access_token")
    
    review_data = {
        "text": "Amazing stay, very clean!",
        "rating": 5,
        "user_id": 1,
        "place_id": 1
    }
    
    # تأكد من المسار (reviews أو places/1/reviews حسب تصميمك)
    response = client.post('/api/v1/reviews/', 
                            json=review_data, 
                            headers={"Authorization": f"Bearer {token}"})
    
    assert response.status_code == 201
    assert response.get_json()["rating"] == 5
